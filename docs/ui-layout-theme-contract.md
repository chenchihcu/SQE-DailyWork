...
| Anomaly management page | Event action menu `案件詳情` / `編輯異常` | `src/ui/widgets/anomaly_management_page.py` / `AnomalyManagementPage` | MainWindow stack, opened from EventListWidget | Main content page, one scroll owner per tab; `案件詳情` is the read-only entry and `編輯異常` enters the same page's edit mode | Four-tab workbench: `案件概覽` / `Action 清單` / `根本原因` / `附件與佐證`（`處理歷程` Tab 已退役；audit 仍背景寫入 `anomaly_audit_logs`）； structured identity header (no stage stepper); overview grid lives on tab 0; Action list (`AnomalyActionTable`) uses column order `狀態 / Action 類型 / 處置內容 / 責任人 / 預定日期 / 編輯 / 流程`, read-only data cells (`text_table_item` with tooltip; `狀態` uses a badge widget), `編輯` button or row double-click opening `EditAnomalyActionDialog`, and separated workflow buttons (`開始執行` / `完成` / `取消` / `新增有效性驗證`); root-cause investigation badges and the single root-cause card live on tab 2; Supplier 8D is managed via attachment category on tab 3; repeat issues route via header button to the dedicated repeat page. Basic anomaly editing embeds `NewAnomalyDialog(embedded=True, page_mode=True)` on tab 0 | Shared theme tokens | `tests/test_anomaly_management_page.py`, `tests/test_anomaly_action_table.py`, `tests/test_repeat_issue_phase5.py`, `tests/test_case_actions_phase1.py`, native `scripts/qt_visual_probe.py --target workbench` at 1.0 / 1.25 / 1.5 |
| Anomaly workbench Action dialogs | Management page Action flows | `src/ui/widgets/anomaly_action_dialog.py` / `edit_anomaly_action_dialog.py` / `complete_action_dialog.py` / `add_verification_dialog.py` / `anomaly_root_cause_dialog.py` (legacy source retained, no product entry: `anomaly_hypothesis_dialog.py`, `anomaly_note_dialog.py`, `add_eight_d_review_dialog.py`, `add_audit_log_dialog.py`) | Anomaly management page | Dialog helper clamps to active screen; one scroll owner and fixed bottom command row | Create captures type, per-row description/owner/due date via `ActionItemListWidget`, execution status, and eligible verification-required choice. Edit uses the same row widget; multi-row save splits legacy packed descriptions into separate Actions. **Root Cause dialog** (`AnomalyRootCauseDialog`): status-first vertical stack inside `create_section_card` panels (`進度` / `核心` / `驗證` / `結論` / conditional `條件`); `根本原因說明` + `驗證證據` use full-width `BulletListWidget`; `驗證方式` uses editable `QComboBox` with `ANOMALY_ROOT_CAUSE_VALIDATION_METHOD_OPTIONS`; `結論說明` + `無法確認原因` use `QTextEdit` (`set_text_edit_visible_rows(3)`); hide the entire `驗證` panel when status is `尚未開始`; show `條件` only for `無法確認`; do not place two `BulletListWidget` fields side-by-side in this dialog. Completion and cancellation remain separate outcomes with completion evidence/note or mandatory cancellation reason. Verification is available only after execution completion. Required-field validation, dirty guard, CJK labels and audit-bundled writes remain mandatory | Shared theme tokens and layout constants | `tests/test_anomaly_workbench_dialogs.py`, `tests/test_anomaly_workbench_write_dialogs.py`, native `scripts/qt_visual_probe.py --target dialog-density` at 1.0 / 1.25 / 1.5 |
...
- `Dynamic Item List (BulletListWidget)`: Structured itemized entries (e.g. defect
  descriptions, tracking points) must use the dynamic numbered row component
  (`BulletListWidget`) to support item-by-item review and dynamic addition
  (`+ 新增條目`), maintaining `\n` line-separated compatibility.
- `Action Item List (ActionItemListWidget)`: Workbench Action create/edit dialogs
  use per-row `[序號] [處置內容] [責任人] [預定日期] [刪除]` rows via
  `ActionItemListWidget`, with a muted column-header row (`處置內容` /
  `責任人` / `預定日期`) above the editable rows (hidden in read-only mode).
  Each non-empty row creates or updates one `case_actions` record with its own
  owner and due date. Editable dialog widths use `ACTION_ITEM_*` tokens from
  `layout_constants.py`; read-only workbench table columns keep `WORKBENCH_ACTION_*`
  (dialog owner/date fields are intentionally wider than table cells). Row rhythm
  uses `ACTION_ITEM_ROW_MIN_HEIGHT` (= `CONTROL_MIN_HEIGHT`), `ACTION_ITEM_ROW_V_MARGIN`,
  and `ACTION_ITEM_HEADER_GAP`; do not hardcode `setMinimumHeight(28)` on row controls.
  Validation uses `ActionItemListWidget.set_validation_invalid()` to mark empty
  description `QLineEdit` children—not `set_field_invalid()` on the list container.
...
- Multi-line narrative fields use `BulletListWidget` (`[序號] [輸入框] [刪除]` + `+ 新增條目`); height grows with item count instead of fixed QTextEdit row caps. The widget-level `placeholder` must appear on the **first row only**; later rows keep `條目 N`. Hide per-row `刪除` when only one row remains (read-only mode still hides all delete buttons).
- Workbench mixed-field dialogs (for example `AnomalyRootCauseDialog`) grade controls by intent: multi-point review → `BulletListWidget`; short method/enumeration → editable `QComboBox`; short summary/conditional note → `QTextEdit`. Do not stack four or more `BulletListWidget` instances in one modal, and do not pair two bullet lists horizontally inside `FORM_MAX_WIDTH` dialogs.
- **Workbench Action list read-only table standard (工作台 Action 清單唯讀表格標準)**: `AnomalyActionTable` displays `狀態 / Action 類型 / 處置內容 / 責任人 / 預定日期 / 編輯 / 流程`; data columns use read-only `text_table_item` with tooltip, `狀態` uses a badge widget, and `編輯` / `流程` host action buttons. Field edits go through `EditAnomalyActionDialog` (`ActionItemListWidget` per-row owner/date). Workflow actions live in the `流程` column. `編輯` / `流程` buttons use `make_table_cell_action_button` (`role="tableCellAction"`) instead of `variant="secondary"` to avoid vertical clipping inside `setCellWidget` rows. Column widths and row height consume `WORKBENCH_ACTION_*` / `TABLE_CELL_*` tokens from `src/ui/layout_constants.py`.
- **Inline table date picker standard (表格內嵌日期選擇器標準)**: reserved for future workbench tables that edit dates inline. `AnomalyActionTable` no longer uses inline date cells; dates are edited in `EditAnomalyActionDialog` only.
...
- Statistics (異常事件統計) is supplier-event only: a dashboard-style page with
  ...
  scrollable multi-row chart grid. Zero-Noise: no insight strip, info banner, or
  ...
  The chart grid covers 供應商事件趨勢, 異常類別柏拉圖, 責任人事件統計,
  重複異常再發率, 產品階段分布, and SMT 製程關鍵字柏拉圖 (row 0: trend +
  category; row 1: responsible + repeat recurrence; row 2: product stage; row 3:
  SMT keyword full width). The removed visit-trend phase, risk / overdue /
  supplier-risk timing charts must not be recreated as visible or hidden page
  widgets. Warehouse nonconforming-product statistics live solely on the
  不合格品統計分析 page (no duplicate warehouse tab here).
...
- Visual-QA coverage + font/chart contract - 2026-06-22: the native probe gained
  ...
  `empty-states`, `pdf-export`, `workbench-empty-analysis`, `repeat-issues-management`) plus `--scale` (multi-DPI), `--min-width`, a
...
| 7 | EmptyStateWidget four-state ready | `src/ui/widgets/common_widgets.py` (`EmptyStateWidget`); used by event list, Master Data, NCR list, statistics pages | `tests/test_layout_constants.py` (`EMPTY_STATE_MARGINS`), `tests/test_master_data_query_behavior.py`, `scripts/qt_visual_probe.py --target empty-states` |
