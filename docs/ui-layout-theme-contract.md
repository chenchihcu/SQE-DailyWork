...
| Anomaly management page | Event action menu `案件詳情` / `編輯異常` | `src/ui/widgets/anomaly_management_page.py` / `AnomalyManagementPage` | MainWindow stack, opened from EventListWidget | Main content page, one scroll owner per tab; `案件詳情` is the read-only entry and `編輯異常` enters the same page's edit mode | Five-tab workbench aligned with `design.md` Archetype 3: `案件概覽` / `Action 清單` / `根本原因` / `附件與佐證` / `處理歷程`; structured identity header (no stage stepper); overview grid lives on tab 0; Action list uses a compact due-date / owner / Open-Done table; root-cause investigation badges and the single root-cause card live on tab 2; Supplier 8D is managed via attachment category on tab 3; repeat issues route via header button to the dedicated repeat page. Basic anomaly editing embeds `NewAnomalyDialog(embedded=True, page_mode=True)` on tab 0 | Shared theme tokens | `tests/test_anomaly_management_page.py`, `tests/test_repeat_issue_phase5.py`, `tests/test_case_actions_phase1.py`, native `scripts/qt_visual_probe.py --target workbench` at 1.0 / 1.25 / 1.5 |
