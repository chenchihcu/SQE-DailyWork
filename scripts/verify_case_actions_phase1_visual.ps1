...
if (-not (Test-Path -LiteralPath $python -PathType Leaf)) {
    throw "Phase 1 visual verification requires .venv\Scripts\python.exe"
}
$formalFingerprintBefore = & $python $fingerprintScript --digest-only $formalDb
if ($LASTEXITCODE -ne 0) {
    throw "Unable to fingerprint the formal database before visual verification."
}

$scratchRoot = Join-Path $repoRoot "scratch\phase1-case-actions-visual"
...
& $python (Join-Path $repoRoot "scripts\sqlite_backup.py") `
    ...
    $verificationDb
if ($LASTEXITCODE -ne 0) {
    throw "Unable to create the verified disposable database for Phase 1 visual verification."
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Disposable case_actions_v1 migration failed before visual verification."
    }

    $targets = @("form-density", "dialog-density", "workbench")
    $scales = @("1.0", "1.25", "1.5")
    ...
    foreach ($target in $targets) {
        foreach ($scale in $scales) {
            & $python (Join-Path $repoRoot "scripts\qt_visual_regress.py") `
                ...
                --min-width
            if ($LASTEXITCODE -ne 0) {
                throw "Phase 1 visual regression failed for $target at scale $scale."
            }
        }
    }

    Write-Host "Phase 1 form-density, workbench, and dialog-density native visual verification passed."
}
finally {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to fingerprint the formal database after visual verification."
    }
    if ($formalFingerprintAfter -ne $formalFingerprintBefore) {
        throw "Formal database mutation detected during Phase 1 visual verification."
    }
}
