"""Preview or apply the explicit Hypothesis Promotion migration.

Preview is read-only. ``--apply`` is intentionally double-gated by
``SQE_ANOMALY_HYPOTHESES_PROMOTION_APPROVED=1`` and
...
startup or Phase 3 verification.
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preview/apply the explicit anomaly_hypotheses_v1 migration."
    )
    parser.add_argument(
        "--db",
        type=Path,
...
def _promotion_marker_enabled() -> bool:
    ...
    return all(
        ...
        for name in (
            "SQE_ANOMALY_HYPOTHESES_PROMOTION_APPROVED",
...
def main() -> int:
    ...
    if not args.apply:
        ...
        try:
            report = preview_anomaly_hypotheses_v1(connection)
        finally:
            connection.close()
        print(
            json.dumps(
    ...
    if not _promotion_marker_enabled():
        raise RuntimeError(
            "Apply refused: SQE_ANOMALY_HYPOTHESES_PROMOTION_APPROVED=1 and "
            "SQE_DAILYWORK_CONFIRM_APPLY=1 are required"
        )

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = target.with_name(
        f"{target.stem}_backup_anomaly_hypotheses_v1_{timestamp}{target.suffix}"
    )
    backup_report = backup_sqlite_database(target, backup_path, verify=True)

    connection = _writable_connection(target)
    try:
        preview = preview_anomaly_hypotheses_v1(connection)
        migration = migrate_anomaly_hypotheses_v1(connection, apply=True)
        connection.commit()
