...
$scratchRoot = Join-Path $repoRoot "scratch\main-visual-baseline"
$verificationDb = Join-Path $scratchRoot "sqe_v2.db"

if (-not (Test-Path -LiteralPath $python -PathType Leaf)) {
    throw "Main visual baseline refresh requires .venv\Scripts\python.exe"
}

$formalFingerprintBefore = & $python $fingerprintScript --digest-only $formalDb
if ($LASTEXITCODE -ne 0) {
    throw "Unable to fingerprint the formal database before main baseline refresh."
}

New-Item -ItemType Directory -Path $scratchRoot -Force | Out-Null
& $python (Join-Path $repoRoot "scripts\sqlite_backup.py") $formalDb $verificationDb
if ($LASTEXITCODE -ne 0) {
    throw "Unable to create the verified disposable database for main baseline refresh."
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Disposable database initialization failed before main baseline refresh."
    }

    $scales = @("1.0", "1.25", "1.5")
    foreach ($scale in $scales) {
        $probePath = Join-Path $scratchRoot "main-$scale.png"
        $probeOutput = & $python (Join-Path $repoRoot "scripts\qt_visual_probe.py") `
            ...
            --output $probePath
        if ($LASTEXITCODE -ne 0) {
            throw "Native main probe failed at scale $scale."
        }
        ...
        if (
            ...
            throw "Native main evidence is not trustworthy at scale $scale."
        }
        Write-Host "Trusted main probe: scale=$scale; platform=$($probe.qt_platform); cjk=$($probe.cjk_font_ok); qss=$($probe.qss_unknown_property_warnings)"
    }
    ...
    foreach ($scale in $scales) {
        & $python (Join-Path $repoRoot "scripts\qt_visual_regress.py") `
            ...
            --update
        if ($LASTEXITCODE -ne 0) {
            throw "Main baseline update failed at scale $scale."
        }
    }
    ...
    foreach ($scale in $scales) {
        & $python (Join-Path $repoRoot "scripts\qt_visual_regress.py") `
            ...
            --min-width
        if ($LASTEXITCODE -ne 0) {
            throw "Updated main baseline did not read back at scale $scale."
        }
    }

    Write-Host "Main baseline refresh and independent read-back passed."
}
finally {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to fingerprint the formal database after main baseline refresh."
    }
    if ($formalFingerprintAfter -ne $formalFingerprintBefore) {
        throw "Formal database mutation detected during main baseline refresh."
    }
}
