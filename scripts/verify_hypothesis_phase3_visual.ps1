...
if (-not (Test-Path -LiteralPath $python -PathType Leaf)) {
    throw "Phase 3 visual verification requires .venv\Scripts\python.exe"
}
$formalFingerprintBefore = & $python $fingerprintScript --digest-only $formalDb
if ($LASTEXITCODE -ne 0) {
    throw "Unable to fingerprint the formal database before Phase 3 visual verification."
}

$scratchRoot = Join-Path $repoRoot "scratch\phase3-hypothesis-visual"
...
& $python (Join-Path $repoRoot "scripts\sqlite_backup.py") `
    ...
    $verificationDb
if ($LASTEXITCODE -ne 0) {
    throw "Unable to create the verified disposable database for Phase 3 visual verification."
}
...
with conn_mod.get_connection() as conn:
    assert anomaly_hypotheses_schema_ready(conn)
print('hypothesis contract ready')
"@
    if ($LASTEXITCODE -ne 0) {
        throw "Disposable hypothesis contract migration failed before visual verification."
    }
    ...
    foreach ($target in $targets) {
        foreach ($scale in $scales) {
            & $python (Join-Path $repoRoot "scripts\qt_visual_regress.py") `
                ...
                --min-width
            if ($LASTEXITCODE -ne 0) {
                throw "Phase 3 visual regression failed for $target at scale $scale."
            }
        }
    }

    Write-Host "Phase 3 dialog-density and workbench native visual verification passed."
}
finally {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to fingerprint the formal database after Phase 3 visual verification."
    }
    if ($formalFingerprintAfter -ne $formalFingerprintBefore) {
        throw "Formal database mutation detected during Phase 3 visual verification."
    }
}
