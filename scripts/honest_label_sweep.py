...
def infer_capability_honest(norm_id: str, current_cap: str) -> str:
    # 1. Priority: Existing valid capability from map
    ...
    if any(
        kw in norm_id for kw in ["deep-research", "reasoning", "thinking", "reasoner"]
    ):
        return "最強推理"
    if any(kw in norm_id for kw in ["flash", "lite", "mini"]):
        return "極速程式"
    if "preview" in norm_id:
        return "精選推理"

    # 3. ABSOLUTELY NO FALLBACK. Return UNKNOWN.
    return "UNKNOWN"
...
def main():
    ...
    for p_key, p_val in providers_data.items():
        ...
        for m_id, m_info in models.items():
            ...
            # Use the HONEST inference
            base_cap = alias_map.get(norm_id, {}).get("canonical_capability", "UNKNOWN")
            capability = infer_capability_honest(norm_id, base_cap)
    ...
    with open(CONFIG_PATH, "w", encoding="utf-8") as f:
        f.write(final_json)
    print(f"Honest Sweep complete. Processed {updated_count} models. No fallback used.")
