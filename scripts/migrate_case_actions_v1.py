"""Preview or apply the explicit case_actions_v1 Promotion migration.

Preview is read-only. ``--apply`` is intentionally double-gated by the
``SQE_CASE_ACTIONS_PROMOTION_APPROVED=1`` and
``SQE_DAILYWORK_CONFIRM_APPLY=1`` environment markers and creates a verified
WAL-safe backup before opening the source for mutation. This entrypoint
must not be called for the formal database until the user has replied with the
exact cross-phase authorization ``繼續``.
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preview/apply the explicit canonical Action migration."
    )
    parser.add_argument(
        "--db",
        type=Path,
...
def main() -> int:
    ...
    if not args.apply:
        ...
        try:
            report = preview_case_actions_v1_migration(connection)
        finally:
            connection.close()
        print(json.dumps({"mode": "preview", "db": str(target), **report}, ensure_ascii=False, indent=2))
        return 0

    if not _promotion_marker_enabled():
        raise RuntimeError(
            "Apply refused: SQE_CASE_ACTIONS_PROMOTION_APPROVED=1 and "
            "SQE_DAILYWORK_CONFIRM_APPLY=1 are required"
        )

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = target.with_name(
        f"{target.stem}_backup_case_actions_v1_{timestamp}{target.suffix}"
    )
    backup_report = backup_sqlite_database(target, backup_path, verify=True)

    connection = _writable_connection(target)
    try:
        preview = preview_case_actions_v1_migration(connection)
        migration = migrate_case_actions_v1(
            connection,
            apply=True,
