...
if (-not (Test-Path -LiteralPath $python -PathType Leaf)) {
    throw "Phase 4 visual verification requires .venv\Scripts\python.exe"
}
$formalFingerprintBefore = & $python $fingerprintScript --digest-only $formalDb
if ($LASTEXITCODE -ne 0) {
    throw "Unable to fingerprint the formal database before Phase 4 visual verification."
}

$scratchRoot = Join-Path $repoRoot "scratch\phase4-workbench-visual"
...
& $python (Join-Path $repoRoot "scripts\sqlite_backup.py") `
    ...
    $verificationDb
if ($LASTEXITCODE -ne 0) {
    throw "Unable to create the verified disposable database for Phase 4 visual verification."
}
...
"@
    if ($LASTEXITCODE -ne 0) {
        throw "Disposable workbench database bootstrap failed before visual verification."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to fingerprint the formal database after Phase 4 visual verification."
    }
    if ($formalFingerprintAfter -ne $formalFingerprintBefore) {
        throw "Formal database mutation detected during Phase 4 visual verification."
    }
}
