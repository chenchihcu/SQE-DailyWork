...
- 移除/重排卡片、對照面板、清單+詳情佈局（例如潛在重複異常管理頁）
...
- **視覺驗收閘門**：佈局/卡片/對照面板變更後必讀 `sqe-dailywork-visual-qa`，並依 `sqe-dailywork-change-router` 的 widget→target 對照跑 native `qt_visual_probe`；未完成 probe 不得標任務完成，禁止以 `Residual risk` 代替 `not verified`。
