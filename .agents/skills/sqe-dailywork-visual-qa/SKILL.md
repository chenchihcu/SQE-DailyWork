...
- `--target` covers every surface family. Non-`main` targets write several suffixed PNGs:
  ...
  - `appearance-settings` — appearance preferences dialog (default & comfortable large)
...
8. **Typography static audit** — no `font-weight: 500|600` in live Qt QSS (theme + ncr); single CJK font source (政策正本:`.Codex/rules/visual_evidence_rules.md` §2). Pinned by tests; re-grep if you touched styling.
...
- 瀏覽器 / web E2E 測試不屬本技能;Playwright 在本專案也不是視覺證據(`.Codex/rules/visual_evidence_rules.md` §1)
