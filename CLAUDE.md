...
- `scripts/qt_visual_probe.py --target main|event-create|workbench|dialog-density|form-density|stats-stress|repeat-issues-management` writes native PNGs — **read the PNGs** for CJK evidence; the console prints CJK as mojibake (cp950 display artifact, not broken data). UI layout changes are **not done** until the mapped target probe passes; do not defer probe to `Residual risk`. The complete target/DPI contract lives in `scripts/qt_probe_targets.json`.
- Multi-page visual review: run **one** `--target` per probe invocation (`main`, `supplier-360`, `event-list`, `manager-view`); targets are defined in `scripts/qt_probe_targets.json` (no `home` target).
...
- Full verify: `scripts/verify.ps1 -Profile Full` includes NCR tests + pytest module regressions after unittest discover; run backgrounded (~15–20 min). Evidence: `scratch/verify-full-log-final.txt`. GitHub Actions Full uses `-AllowSchemaOnlySource -SkipNativeVisual` (no formal DB in checkout; not visual evidence). CI also sets `PYTHONUNBUFFERED=1`, unittest `-v`, and `SQE_TEST_HANG_SECONDS=180`; a cancelled job is not a green gate.
