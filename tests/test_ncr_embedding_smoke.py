...
class NcrEmbeddingSmokeTests(unittest.TestCase):
    ...
    def test_ncr_widgets_at_expected_indices(self) -> None:
        ...
        outsource_page = self.window.stack.widget(NCR_PENDING_OUTSOURCE_PAGE_INDEX)
        material_page = self.window.stack.widget(NCR_PENDING_MATERIAL_PAGE_INDEX)
        history_page = self.window.stack.widget(NCR_TRACE_PAGE_INDEX)

        self.assertIsNotNone(create_page.findChild(DefectFormWidget))
        outsource_widget = outsource_page.findChild(DefectListWidget)
        material_widget = material_page.findChild(DefectListWidget)
        history_widget = history_page.findChild(DefectListWidget)
        self.assertIsNotNone(outsource_widget)
        self.assertIsNotNone(material_widget)
        self.assertIsNotNone(history_widget)
        self.assertEqual("tracking", outsource_widget.workflow)
        self.assertEqual("tracking", material_widget.workflow)
        self.assertEqual(PROCESSING_LINE_OUTSOURCE, outsource_widget.processing_line)
        self.assertEqual(PROCESSING_LINE_MATERIAL, material_widget.processing_line)
        self.assertIn(
    ...
    def test_can_switch_into_every_ncr_page(self) -> None:
        actions = [
            ...
            ("page", PAGE_NCR_PENDING_OUTSOURCE),
            ("page", PAGE_NCR_PENDING_MATERIAL),
