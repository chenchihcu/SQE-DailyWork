...
class AnomalyActionsRepositoryTests(unittest.TestCase):
    ...
    def test_create_action_normalizes_due_date_to_iso(self) -> None:
        ...
        row = repository.get_case_action(self.conn, action_id)
        assert row is not None
        self.assertEqual(row["due_date"], "2026-07-01")
        self.assertEqual(row["execution_status"], "執行中")
        self.assertEqual(row["owner"], "Alice")

    def test_create_action_rejects_invalid_status(self) -> None:
        with self.assertRaisesRegex(ValueError, "New Action status"):
            repository.create_case_action(
                self.conn,
                anomaly_id=self.anomaly_id,
                action_type="NEXT_ACTION",
    ...
    def test_list_orders_open_first_then_by_due_date(self) -> None:
        ...
        no_date_id = repository.create_case_action(
            self.conn,
            anomaly_id=self.anomaly_id,
            action_type="NEXT_ACTION",
            description="nodate",
    ...
    def test_complete_sets_status_and_timestamp(self) -> None:
        ...
        row = repository.get_case_action(self.conn, action_id)
        assert row is not None
        self.assertEqual(row["execution_status"], "已完成")
        self.assertEqual(row["completed_at"], "2026-07-02")
        self.assertEqual(row["completion_note"], "FA submitted")
    ...
    def test_cancel_sets_status_and_timestamp(self) -> None:
        ...
        row = repository.get_case_action(self.conn, action_id)
        assert row is not None
        self.assertEqual(row["execution_status"], "已取消")
        self.assertEqual(row["cancelled_at"], "2026-07-01")
        self.assertEqual(row["cancel_note"], "supplier delayed")
    ...
    def test_overdue_ignores_cancelled_actions(self) -> None:
        ...
        repository.cancel_case_action(self.conn, action_id, cancel_note="x")
        self.assertFalse(
            repository.is_case_action_overdue(
                self.conn, self.anomaly_id, today="2026-08-01"
            )
    ...
    def test_current_action_picks_earliest_due(self) -> None:
        ...
        early_id = repository.create_case_action(
            self.conn,
            anomaly_id=self.anomaly_id,
            action_type="NEXT_ACTION",
            description="early",
        ...
        current = repository.get_current_case_action(
    ...
    def test_current_action_picks_open_over_completed(self) -> None:
        completed_id = repository.create_case_action(
            self.conn,
            anomaly_id=self.anomaly_id,
            action_type="NEXT_ACTION",
            description="done",
        ...
        repository.complete_case_action(self.conn, completed_id)
        open_id = repository.create_case_action(
            self.conn,
            anomaly_id=self.anomaly_id,
            action_type="NEXT_ACTION",
            description="open",
...
class AnomalyActionsMigrationTests(unittest.TestCase):
    ...
    def test_legacy_backfill_stays_disabled_after_canonical_promotion(self) -> None:
        ...
        self.conn.commit()
        # Canonical promotion makes the legacy table an immutable snapshot.
