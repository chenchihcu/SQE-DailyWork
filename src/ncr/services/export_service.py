...
def export_ncr_excel_report(
    ...
    try:
        ...
        # 報告標題與時間資訊
        report_sheet.cell(row=1, column=1, value="倉庫不合格品統計分析報告").font = styles["STYLE_TITLE_FONT"]
        ...
        report_sheet.cell(row=2, column=1, value=subtitle_text).font = styles["STYLE_SUBTITLE_FONT"]
        ...
        report_sheet.cell(row=4, column=1, value="📊 不合格品總量統計").font = styles["STYLE_FONT_BOLD"]
        report_sheet.cell(row=4, column=1).alignment = styles["ALIGN_CENTER"]
        report_sheet.cell(row=4, column=1).fill = styles["STYLE_FILL_KPI_BG"]

        report_sheet.cell(row=5, column=1, value=f"總件數: {total_cases} 件").font = styles["STYLE_FONT"]
        report_sheet.cell(row=5, column=1).alignment = styles["ALIGN_CENTER"]
        report_sheet.cell(row=5, column=2, value=f"總數量: {total_qty} 件").font = styles["STYLE_FONT"]
        report_sheet.cell(row=5, column=2).alignment = styles["ALIGN_CENTER"]

        # 卡片二：處置卡片
        report_sheet.merge_cells("E4:G4")
        report_sheet.cell(row=4, column=5, value="⚙️ 處置方式與損失率").font = styles["STYLE_FONT_BOLD"]
        report_sheet.cell(row=4, column=5).alignment = styles["ALIGN_CENTER"]
        report_sheet.cell(row=4, column=5).fill = styles["STYLE_FILL_KPI_BG"]

        report_sheet.cell(row=5, column=5, value=f"報廢數量: {scrap_qty} 件 ({scrap_rate:.1f}%)").font = styles["STYLE_FONT"]
        report_sheet.cell(row=5, column=5).alignment = styles["ALIGN_CENTER"]
        report_sheet.cell(row=5, column=6, value=f"重工數量: {rework_qty} 件").font = styles["STYLE_FONT"]
        report_sheet.cell(row=5, column=6).alignment = styles["ALIGN_CENTER"]

        # 套用卡片框線
        for r in [4, 5]:
            for c in [1, 2, 3, 5, 6, 7]:
                report_sheet.cell(row=r, column=c).border = styles["STYLE_BORDER_THIN"]
        ...
        for col_idx in range(1, len(DETAIL_EXPORT_HEADERS) + 1):
            ...
            cell.font = styles["STYLE_HEADER_FONT"]
            cell.fill = styles["STYLE_FILL_HEADER"]
            cell.alignment = styles["ALIGN_CENTER"]
            cell.border = styles["STYLE_BORDER_THIN"]
        ...
        for r_idx, row in enumerate(defect_rows, start=start_row):
            ...
            for c_idx in range(1, len(DETAIL_EXPORT_HEADERS) + 1):
                ...
                cell.font = styles["STYLE_FONT"]
                cell.border = styles["STYLE_BORDER_THIN"]
                if is_even:
                    cell.fill = styles["STYLE_FILL_ZEBRA"]
        ...
        if len(defect_rows) > 0:
            detail_sheet.cell(row=total_row_idx, column=1, value="合計").font = styles["STYLE_FONT_BOLD"]
            detail_sheet.cell(row=total_row_idx, column=1).alignment = styles["ALIGN_CENTER"]
            ...
            qty_cell.font = styles["STYLE_FONT_BOLD"]
            qty_cell.alignment = styles["ALIGN_RIGHT"]
            qty_cell.number_format = "#,##0"

            # 設定合計行底線樣式
            for c_idx in range(1, len(DETAIL_EXPORT_HEADERS) + 1):
                cell = detail_sheet.cell(row=total_row_idx, column=c_idx)
                cell.border = styles["STYLE_BORDER_TOTAL"]
                cell.fill = styles["STYLE_FILL_TOTAL"]
        ...
        for section_title, name_key in STATS_SECTION_DEFINITIONS:
            ...
            # 分區大標題
            stats_sheet.cell(row=current_row, column=1, value=f"📋 {section_title}").font = styles["STYLE_FONT_BOLD"]
            ...
            # 分區合計列
            if len(rows) > 0:
                stats_sheet.cell(row=current_row, column=1, value="合計").font = styles["STYLE_FONT_BOLD"]
                stats_sheet.cell(row=current_row, column=1).alignment = styles["ALIGN_CENTER"]
                ...
                for col_idx in range(2, len(headers) + 1):
                    ...
                    cell.font = styles["STYLE_FONT_BOLD"]
                    cell.alignment = styles["ALIGN_RIGHT"]
