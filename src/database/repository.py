...
def get_anomaly_trend(conn: sqlite3.Connection, months: int = 6) -> list[dict]:
    """Fetch monthly anomaly counts, closed counts, backlog, and overdue counts for the last N months."""
    now = datetime.now()
    months_list = []
    ...
    for yyyymm in months_list:
        # 1. Total created in this month
        total_row = conn.execute(
            "SELECT COUNT(*) as c FROM anomalies WHERE substr(anomaly_date, 1, 7) = ?",
            (yyyymm,)
        ).fetchone()
        
        # 2. Total closed in this month (using closed_at)
        closed_row = conn.execute(
            "SELECT COUNT(*) as c FROM anomalies WHERE substr(closed_at, 1, 7) = ?",
            (yyyymm,)
        ).fetchone()

        # 3. Currently overdue (created in this month and still pending past due_date)
        ...
        # 4. Cumulative backlog at the end of this month
        # Logic: (Total created all-time <= M) - (Total closed all-time <= M)
        # Using closed_at to determine if it was closed by then.
