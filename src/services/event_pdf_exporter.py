...
def build_event_pdf_html(
    ...
    *,
    ...
    anomaly_fields: list[tuple[str, object]] = [
        ...
        ("批量數", _positive_number_or_dash(detail.get("batch_qty"))),
        ("檢驗數", _positive_number_or_dash(detail.get("qty_inspected"))),
        ("不良數", _positive_number_or_dash(detail.get("qty_ng"))),
        ("不良率", detail.get("defect_rate_display") or "—"),
...
def export_event_pdf(
    ...
    return _export_html_pdf(
        path,
        lambda: build_event_pdf_html(row, detail),
        success_prefix="已匯出至：",
...
def export_brief_event_pdf(
    ...
    return _export_html_pdf(
        path,
        lambda: build_brief_event_pdf_html(row, detail),
...
def render_brief_event_to_image(
    ...
    try:
        ...
        html = build_brief_event_pdf_html(row, detail)
...
def _quality_overview_section(overview: dict, open_actions: list[dict]) -> str:
    ...
    from services.event._anomaly_markdown import OVERVIEW_FIELDS

    overview_labels = dict(OVERVIEW_FIELDS)
    fields: list[tuple[str, object]] = [
        (overview_labels["overdue"], "是" if overview.get("overdue") else "否"),
        (
            overview_labels["current_action_text"],
            _current_action_display(overview.get("current_action")),
        ),
        (
            overview_labels["open_action_count"],
            int(overview.get("open_action_count") or 0),
        ),
        (
            overview_labels["root_cause_status"],
            overview.get("root_cause_status") or "尚未開始",
        ),
        (
            overview_labels["corrective_action_status"],
            overview.get("corrective_action_status") or "—",
        ),
        (
