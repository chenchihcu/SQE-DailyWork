...
def _html_document(
    ...
    font_size_css = "8.5pt" if getattr(prefs, "pdf_font_density", "standard") == "compact" else "10pt"
    org_header = getattr(prefs, "report_organization_header", "") or "MITCORP / SQE Quality Report"
    include_disclaimer = getattr(prefs, "export_include_disclaimer", True)
    disclaimer_suffix = " &nbsp;&bull;&nbsp; 內部專用品質文件，未經授權禁止外傳" if include_disclaimer else ""
...
{body}
<div class="footer">
  <span class="footer-text">{escape(org_header)} &nbsp;&bull;&nbsp; Generated by SQE DailyWork{disclaimer_suffix}</span>
