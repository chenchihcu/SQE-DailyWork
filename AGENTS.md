...
- **Trace-number uniqueness (same supplier)**: Same `supplier_id` + same trace column + same non-empty value → reject create/update (`ValueError` with conflicting `anomaly_no`). No UNIQUE index restore or bulk legacy cleanup; grandfather unchanged values on update. Details: `docs/architecture-workflow-contract.md`.
- **Anomaly quantity fields and defect rate**: `batch_qty`, optional `qty_inspected` (default 0), `qty_ng`; `defect_rate` denominator = `qty_inspected` if >0 else `batch_qty` (`repo_helpers` SSOT); `qty_ng` ≤ active denominator; NCR handoff pre-fills `qty_ng` only.
...
- **Anomaly Number Format and Editing Constraints**: `anomaly_no` / `ref_no` is exactly 11 digits (`YYYYMMDDNNN`); prefix must match selected date. On date change, regenerate preview only when `auto_fill_anomaly_no_on_date_change` is true; initial create/load still previews once. Submit validates uniqueness, length, and date-prefix alignment. Test mocks must match mock date.
...
- **Itemized Description Standard (條列式逐條審閱動線)**: Descriptions, defect findings, and tracking items must use the dynamic numbered row component `BulletListWidget` (`[序號] [輸入框] [刪除]` + `+ 新增條目`) to facilitate item-by-item review while maintaining newline-delimited text compatibility. Wire `BulletListWidget(placeholder=...)` to the first row input; hide the row `刪除` button when only one row remains.
- **Workbench Root Cause Dialog Field Grading (工作台根本原因欄位分級)**: `AnomalyRootCauseDialog` is status-first with `create_section_card` sections. Use `BulletListWidget` for `根本原因說明` and `驗證證據` only; `驗證方式` is an editable combo fed by `ANOMALY_ROOT_CAUSE_VALIDATION_METHOD_OPTIONS`; `結論說明` and conditional `無法確認原因` are `QTextEdit` summaries. Hide the entire `驗證` section when status is `尚未開始`. Never place two `BulletListWidget` fields side-by-side in this dialog.
...
- **Open-date calendar guard**: `NewAnomalyDialog.date_edit` and NCR `event_date_edit` use `setMaximumDate(QDate.currentDate())`; `_ensure_date_not_in_future` remains the service backstop.
...
- **UI visual closure gate**: When changing visible `src/ui/` layout, typography, CJK copy, cards, or comparison panels, delivery is **not complete** until the mapped `qt_visual_probe --target` passes on native Windows (`visual_trustworthy: true`, `cjk_font_ok: true`, `qss_unknown_property_warnings: 0`) and the saved PNG is read for the touched surface. Offscreen unittest/structural smoke does not satisfy this gate. Widget→target mapping lives in `.claude/skills/sqe-dailywork-change-router/SKILL.md` and `docs/harness/test-patterns.md` §4.
- **Residual risk vs not verified**: Do not list skipped mandatory checks (especially native visual probe) under `Residual risk`. If a required check was not run or failed, label the task **`not verified`** or keep it open. `Residual risk` is only for limitations that remain **after** the required checks were executed (for example font install variance or non-Windows hosts that cannot run native Qt).
