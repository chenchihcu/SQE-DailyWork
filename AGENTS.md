...
- **Statistics Chart and List Category Alignment (including Dialogs)**: When updating statistics charts (e.g. Pareto chart), listings, reports, or management dialogs, ensure category display and preselection logic remains consistent across the system. 
  1. If a Pareto chart prioritizes `root_cause_category` over `category` for closed anomalies, the event lists, Excel export detail tables, and **the read-only anomaly dialog (when loading a closed event)** must also prioritize and display `root_cause_category` over `category` to align visual terminology.
