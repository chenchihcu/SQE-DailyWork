...
- **Windows chunked unittest**: Full and Coverage use `Invoke-UnittestDiscoverWindowsSafe` (Coverage: 4 chunks, `PYTHONUNBUFFERED=1`); never single discover as Windows Coverage evidence; see §4 tearDown.
- **Coverage evidence chain**: `assert_coverage_baseline.py` PASS alone does not mean `-Profile Coverage` PASS. A valid claim requires `scratch/verify-coverage-*-final*.log` with `EXIT:0` and the full chain visible in the log: `compileall` → coverage chunks → `pytest` → `assert_coverage_baseline.py` → `harness_check.ps1`. Do not cite `coverage-summary.json` or a single sub-step as profile PASS.
