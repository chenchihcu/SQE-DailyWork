...
- **Statistics Chart and List Category Alignment**: When updating statistics charts (e.g. Pareto chart) and detail listings or export reports, ensure that the category aggregation logic used by the charts aligns exactly with the category display logic used by the lists, tables, and reports. For example, if a Pareto chart prioritizes `root_cause_category` over `category` for closed anomalies, the event lists and Excel export detail tables must also prioritize `root_cause_category` over `category` when querying closed anomalies.
