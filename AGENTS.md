...
- **Statistics Chart and List Category Alignment (including Dialogs)**: When updating statistics charts (e.g. Pareto chart), listings, reports, or management dialogs, ensure category display and preselection logic remains consistent across the system. 
  1. If a Pareto chart prioritizes `root_cause_category` over `category` for closed anomalies, the event lists and Excel export detail tables must also prioritize and display `root_cause_category` over `category` to align visual terminology. In the anomaly dialog (both edit and read-only preview), if the anomaly is closed, it must display the raw `category` as "異常類別" and the `root_cause_category` as a read-only "原因分類" field to clearly show both values and match the list's categorization.
  2. When closing an anomaly in the **close anomaly dialog**, the "cause category" (`root_cause_category`) combobox must pre-populate and default to the anomaly's current "category" to prevent accidental overrides.
  3. Category resolution (`COALESCE(root_cause_category, category)`) is **display-only**. Anomaly dialogs (both edit and read-only preview) must load the raw `category` (`category_raw` from `get_anomaly_detail`) as "異常類別", never the resolved value — otherwise saving silently writes the root-cause value back into the `category` column. The page Pareto chart, the Excel export Pareto table, and the export-embedded chart PNG must all consume the single `get_anomaly_category_pareto_by_range` implementation, and Python post-processing that merges normalized SQL groups must accumulate counts rather than overwrite dict keys.
